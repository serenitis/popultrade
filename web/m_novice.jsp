...
if (request.getParameter("akcija")!=null && request.getParameter("akcija").equals("DELETE"))  {
	...
	dao.removeNovice(new Long(request.getParameter("id")));
 
	
}
com.popultrade.model.Novice sif = new com.popultrade.model.Novice();
 
List lis =null;
 
lis = dao.getNoviceAll("");
...
collapse; font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	   ...
            	   <td width=65% align=left bgcolor="D2EAFF" onmouseout="popupoff()" onmousemove="popupon(p6,document.all||document.layers?event:null,'')">Naslov novice</td>
               
            	   
            	   <td width=10% align=center bgcolor="ADD9FF" onmouseout="popupoff()" onmousemove="popupon(p8,document.all||document.layers?event:null,'')">Status</td>
               
            	   
            	   <td width=15% align=center bgcolor="D2EAFF" onmouseout="popupoff()" onmousemove="popupon(p10,document.all||document.layers?event:null,'')">Datum Novice</td>
               
            	   
 
              
                 <td  bgcolor="ADD9FF" align=center><a onClick="return godown('m_novice_edit.jsp?opcija=new&siframm=new&sifrant=null')" href="Javascript: PGAPP()"><img alt="Dodaj novo Novico" border="0" src="vnos.gif" /></a></td>
        </tr>
...
while (it.hasNext()){  // while start
	...
	com.popultrade.model.Novice vseb = (com.popultrade.model.Novice)(it.next());
		stvrstic++;
	if (stvrstic>200) {
		break;
	}
...
%>
	...
	<td width="65%" align="left"><%=nul.jeNull(vseb.getNaslov_novice())%></td>
	

	<td width="10%" align="center"><%=nul.jeNull(vseb.getStatus())%></td>
	
	<td width="15%" align="center"><%=nul.getDatumFormatFromDateNoTime(vseb.getDatum_novice())%></td>
...
<td  align="center">
 ...
 <a href="Javascript:PGAPP()" onCLick="return go('<%=vseb.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&sifrant=<%=request.getParameter("sifrant")%>',1)"><img src="u.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" alt="Spremeni podatke"></a>
  ...
  <a onClick="return godown('m_kat1_slike.jsp?prikazi=1&sifra=<%=vseb.getId()%>&sifraor=null&stsl=22&back=')" href="Javascript: vod()"  onmouseout="popupoff()" onmousemove="popupon('Brisi, dodaj slike',document.all||document.layers?event:null)"><img src="i.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" alt="Spremeni slike"></a>
 
 
 <a onClick="return vprasaj()" href="m_novice.jsp?akcija=DELETE&id=<%=vseb.getId()%>&sifrant=<%=request.getParameter("sifrant")%>"><img src="x.gif" border=0 alt="Brisi"></a>
...
<td bgcolor=silver align=center><a href="m_novice.jsp?a=<%=f%>&b=<%=f+stpri+1%><%=iskk%>"> <%=i%> </a></td>
...
<td bgcolor=silver align=center><a href="m_novice.jsp?a=<%=f%>&b=<%=ost+f+1%><%=iskk%>"> <%=i%> </a></td>
