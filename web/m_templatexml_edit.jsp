...
com.popultrade.dao.TemplateXmlDAO dao = (com.popultrade.dao.TemplateXmlDAO)contextUtil.getBeanDao("templateXmlDAO",pageContext.getServletContext());
if (request.getParameter("id")!=null && request.getParameter("akcijaXY")==null) {
vseb = dao.getTemplateXml(new Long(nul.jeNullDefault(request.getParameter("id"))));
}
...
vseb = dao.getTemplateXml(new Long(nul.jeNullDefault(request.getParameter("id"))));
nidd=vseb.getId();
vseb = (com.popultrade.model.TemplateXml)contextUtil.getPopulatedObject(vseb,request);
}
else {
akcij=5;
String rezu="";
vseb = (com.popultrade.model.TemplateXml)contextUtil.getPopulatedObject(vseb,request);
vseb.setId_nad(new Long(request.getParameter("ida")));
vseb.setId_conversion(new Long(request.getParameter("idg")));
}
if (rabiinsert.equals("") && !bok) {
dao.saveTemplateXml(vseb);
nidd=vseb.getId();
}
}
}
...
%>
<form name=dod action="m_templatexml_edit.jsp" method="post"    onSubmit="return custom()">
<%
...
prisotnost.addUserNarocilo(control.idUporabnikaLock,nidd+"",new Date(),akcij,"TemplateXml",control.getUserName(),"");
%>
...
parent.frames[0].odpriUL('<%=request.getParameter("ida")%>');
