...
<iframe noresize="noresize"  id="ifr2" name="prvain" target="drugain"  scrolling="yes" frameborder="0"   src="<%=request.getParameter("lokacijaa").replaceAll("QUE","?").replaceAll("AND","&")%>"   style="width:100%;height:100%"  >b</iframe>
        </div>
...
<iframe noresize="noresize" id="ifr3" name="drugain" target="prvain" src="nic.jsp" scrolling="yes" frameborder="0" style="width:100%;height:100%"  onload="collapseL(this.contentWindow.location)"  >b</iframe>
     ...
     $(document).ready(function () {
         ...
         $('#mainSplitter').jqxSplitter({ theme: theme, width: '100%', height: '100%', orientation: 'horizontal', panels: [{ size: '<%=request.getParameter("okno11")%>%',collapsible:false }, { size: '<%=request.getParameter("okno22")%>%',collapsible:true}] });
