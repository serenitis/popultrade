...
int stpri = control.st_vrstic+120;
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	...
            	<td width=30% align=center bgcolor="D2EAFF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p13)">Vred.</td>
                 ...
                 <a onClick="return godown('m_templateclass_pogoj_edit.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=1&idg=<%=session.getAttribute("idg")%>')" href="Javascript: PGAPP()"><img title="Dodaj nov TemplateClass" border="0" src="vnos.gif" /></a>
...
%>
	...
	<td width="30%" align="left"><%=nul.jeNull(vseb.getVrednost_pogoja()).replace("<!--","").replace("-->","")%></td>
...
String rezu="<table border='0' cellspacing='1'   class='fonti'   width='100%'><tr><td style='width:50%'  class='opiscell'>"+ConPool.getPrevod("5993P"+control.getJezik()).replaceAll("_"," ").replaceAll("<!--","").replaceAll("-->","")+"</td><td style='width:50%'  class='opiscell'>"+ConPool.getPrevod("5996P"+control.getJezik()).replaceAll("_"," ").replaceAll("<!--","").replaceAll("-->","")+"</td></tr></table>";
//}
%>
 <a onClick="return vprasaj()" href="m_templateclass_pogoj.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iu.hasNext()) {
	...
	<td  class="<%=colors[br]%>" width="70%" ><%=nul.jeNull(tcv.getVrednost()).replace("<!--","").replace("-->","")%>
	</td>
		<td  class="<%=colors[br]%>" >
		
		
	<a onClick="return godown('m_templateclass_pogoj_edit2.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
	...
	while (limo.hasNext()) {
		...
		<tr class="<%=colors[br]%>"  onMouseOver="colors(this,'rollOver')" onMouseOut="colors(this,'<%=colors[br]%>')">
			...
			<td width="30%" align="left"><%=nul.jeNull(si.getVrednost_pogoja()).replace("<!--","").replace("-->","")%></td>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_pogoj_edit2.jsp?id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Spremeni podatke"></a>
	
		 <a onClick="return vprasaj()" href="m_templateclass_pogoj.jsp?akcija=DELETE&id=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
