...
if (request.getParameter("akcija")!=null && request.getParameter("akcija").equals("DELETE"))  {
	...
	dao.removeTemplateAttribute(new Long(request.getParameter("id")));
...
if (request.getParameter("pagenumber")!=null && !request.getParameter("pagenumber").equals("")) {
	...
	session.putValue("pagenumberTemplateAttribute",pagenum+"");
	}
	else {
	if (session.getValue("pagenumberTemplateAttribute")!=null) {
	pagenum = Integer.parseInt(session.getValue("pagenumberTemplateAttribute")+"");
	}
	}


com.popultrade.model.TemplateAttribute sif = new com.popultrade.model.TemplateAttribute();

sif.setId_nad(new Long(request.getParameter("id_nad")));

int strec = dao.getStTemplateAttribute(sif,"");



List lis =null;

lis = dao.getTemplateAttributes(sif,pagenum,stizpisov,"","zaporedje","asc");
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
 ...
 <td width=35% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()"  ondragover="allowDrop(event)"  ondrop="drop('TemplateAttribute','')"  onmousemove="ddrivetip(p1)">Tag Name</td>

            	   <td width=45% align=center bgcolor="D2EAFF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p3)">Description</td>
               
            	   
         	   <td width=10% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p4)">Seq</td>       	
                 ...
                 <a onClick="return godown('m_templatexml_att_edit.jsp?opcija=new&siframm=new&sifrant=null&id_nad=<%=request.getParameter("id_nad") %>&idg=<%=request.getParameter("idg") %>&ida=<%=request.getParameter("ida") %>')" href="Javascript: PGAPP()"><img title="Add new attribute" border="0" src="vnos.gif" /></a>
...
while (it.hasNext()){  // while start
	...
	com.popultrade.model.TemplateAttribute vseb = (com.popultrade.model.TemplateAttribute)(it.next());
		stvrstic++;
...
%>
<td width="35%" align="left" draggable="true"  ondragover="allowDrop(event)"  ondrop="drop('TemplateGumbi','&_Ltemplatehead_id=<%=vseb.getId() %>')"  ondragstart="dragStart(event,'TemplateAttribute','tem12x','&id=<%=vseb.getId()%>')" >



<%=nul.jeNull(vseb.getTag_name())%></td>
	<td width="45%" align="left"><%=nul.jeNull(vseb.getDescription())%></td>
...
<td  align="center">
 ...
 <a href="Javascript:PGAPP()" onCLick="return go('<%=vseb.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&sifrant=<%=request.getParameter("sifrant")%>&id_nad=<%=request.getParameter("id_nad") %>&idg=<%=request.getParameter("idg") %>&ida=<%=request.getParameter("ida") %>',1)"><img src="u.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Change data (ID: <%=vseb.getId()%>)"></a>


 <a onClick="vprasaj()?document.location='m_templatexml_att_list.jsp?akcija=DELETE&id_nad=<%=request.getParameter("id_nad") %>&id=<%=vseb.getId()%>&idg=<%=request.getParameter("idg") %>&ida=<%=request.getParameter("ida") %>':''" href='#'><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatexml_att_edit.jsp?id="+asd;
...
int prvii=nul.jeNull(session.getValue("pagenumberTemplateAttribute")+"").equals("")?1:Integer.parseInt(session.getValue("pagenumberTemplateAttribute")+"");
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateAttribute")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_conversions.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateAttribute")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_conversions.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
<td>|</td>
<td >
page: <%=nul.jeNull(session.getValue("pagenumberTemplateAttribute")+"").equals("")?"1":session.getValue("pagenumberTemplateAttribute")%> of <%=ststr+1%> </td>
<td>|</td>
<td>
records: <%=strec%></td>
</tr>
