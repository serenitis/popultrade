...
com.popultrade.dao.TemplateUiDAO dao2 = (com.popultrade.dao.TemplateUiDAO)contextUtil.getBeanDao("templateUiDAO",pageContext.getServletContext());
...
sif = new com.popultrade.model.TemplateUi();
...
sif.setTab_index(request.getParameter("SI_"+imo));
sif.setVnos_kdaj("BO");
sif.setTip_inserta("textbox");


String dolz = request.getParameter("X"+name);
System.out.println("DOLZ ::: "+dolz);

if (dolz!=null && !dolz.trim().equals("")) {

	sif.setStolpec_max_stevilo_znakov(dolz);
	
	if (Integer.parseInt(dolz)>70) {
	sif.setStolpec_sirina("70");
	}
	else {
		sif.setStolpec_sirina(""+(Integer.parseInt(dolz)+1));
	}
}
...
<script language=javascript>
  ...
  document.location = "m_templateui.jsp?idt=<%=request.getParameter("idt")%>";
...
<form name=dod action="m_templateui_avt.jsp" method="post" onsubmit="return chek()" >
