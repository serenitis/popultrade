...
com.popultrade.model.TemplateConversion vseb =new com.popultrade.model.TemplateConversion();
...
vseb = dao.getTemplateConversion(new Long(nul.jeNullDefault(request.getParameter("id"))));
}
...
vseb = dao.getTemplateConversion(new Long(nul.jeNullDefault(request.getParameter("id"))));
nidd=vseb.getId();
vseb = (com.popultrade.model.TemplateConversion)contextUtil.getPopulatedObject(vseb,request);
}
else {
akcij=5;
String rezu="";
...
dao.saveTemplateConversion(vseb);
nidd=vseb.getId();
}
}
}
...
%>
<form name=dod action="m_conversion_edit.jsp" method="post"    onSubmit="return custom()">
<%
...
<td class="opiscell" width="20%" align="left">Status (Disabled/Enabled):</td><td class="valuecell" width="30%">
<%
rezu=nul.jeNull(vseb.getStatus()+"");

%>


	 <select name="status"   tabindex='12'  class="inputIEx" >
					<option value="0" <%=nul.jeNull(rezu).equals("0")?"selected":""%>>Disabled</option>
					<option value="1" <%=nul.jeNull(rezu).equals("1")?"selected":""%>>Enabled</option>
					</select>
...
<td class="opiscell" width="20%" align="left">Company ID (EAN):</td><td class="valuecell"  width="30%" ><%
rezu=nul.jeNull(vseb.getCompany_id()+"");

%>
<input type="text"    tabindex='19' id="company_id"  name="company_id"     value="<%=rezu%>"    class="inputIEx"   >
</td>
</tr>
...
function custom() {
	...
	if (trim(document.dod.company_id.value)=="") {
		
		alert("Error: company id is not entered!");
		return false;
	}
...
prisotnost.addUserNarocilo(control.idUporabnikaLock,nidd+"",new Date(),akcij,"TemplateConversion",control.getUserName(),"");
%>
