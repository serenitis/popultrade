...
if (request.getParameter("akcija")!=null && request.getParameter("akcija").equals("DELETE"))  {
	...
	dao.removeTemplateMenun(new Long(request.getParameter("id")));
...
if (request.getParameter("pagenumber")!=null && !request.getParameter("pagenumber").equals("")) {
	...
	session.putValue("pagenumberTemplateMenun",pagenum+"");
	}
	else {
	if (session.getValue("pagenumberTemplateMenun")!=null) {
	pagenum = Integer.parseInt(session.getValue("pagenumberTemplateMenun")+"");
	}
	}

com.popultrade.model.TemplateMenun sif = new com.popultrade.model.TemplateMenun();
...
if (nul.jeNull(request.getParameter("isci")).equals("yes")) {
	iskanje = true;
	//sif = (com.popultrade.model.TemplateMenun)contextUtil.getPopulatedObject(sif,request);
	...
	session.setAttribute("TemplateMenun_search",sif);
	
}
if (nul.jeNull(request.getParameter("isk")).equals("1")) {
iskanje = true;
sif = (com.popultrade.model.TemplateMenun)session.getAttribute("TemplateMenun_search");

}

int strec = dao.getStTemplateMenun( sif,"");
...
lis = dao.getTemplateMenuns(sif,pagenum,stizpisov,"","name","desc");
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	   ...
            	   <td width=95% align=center bgcolor="D2EAFF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p1)">Menu Name</td>
                 ...
                 <a onClick="return godown('m_templatemenun_edit.jsp?opcija=new&siframm=new&sifrant=null')" href="Javascript: PGAPP()"><img title="Dodaj nov menu" border="0" src="vnos.gif" /></a></td>
        </tr>
...
while (it.hasNext()){  // while start
	...
	com.popultrade.model.TemplateMenun vseb = (com.popultrade.model.TemplateMenun)(it.next());
		stvrstic++;
...
%>
	...
	<td width="95%" align="left"><%=nul.jeNull(vseb.getName())%> </td>
...
<td  align="center">
 ...
 <a onClick="return vprasaj()" href="m_templatemenun.jsp?akcija=DELETE&id=<%=vseb.getId()%><%=(nul.jeNull(request.getParameter("isk")).equals("1") || nul.jeNull(request.getParameter("isci")).equals("yes"))?"&isk=1":""%>"><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatemenun_edit.jsp?id="+asd;
...
<td align=center >
Browse page: </td><td>
 <a style="text-decoration:none;color:black"  href="m_templatemenun.jsp?pagenumber=1<%=iskk%>">first</a></td><td>|</td>
...
int prvii=nul.jeNull(session.getValue("pagenumberTemplateMenun")+"").equals("")?1:Integer.parseInt(session.getValue("pagenumberTemplateMenun")+"");

if (prvii>1) {
int prej = prvii-1;
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatemenun.jsp?pagenumber=<%=prej%><%=iskk%>">previous</a></td>
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateMenun")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatemenun.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateMenun")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatemenun.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatemenun.jsp?pagenumber=<%=prej%><%=iskk%>">next</a></td>
...
<td><a style="text-decoration:none;color:black"  href="m_templatemenun.jsp?pagenumber=<%=ststr+1%><%=iskk%>">last</a></td>
<td>|</td>
<td >
page: <%=nul.jeNull(session.getValue("pagenumberTemplateMenun")+"").equals("")?"1":session.getValue("pagenumberTemplateMenun")%> of <%=ststr+1%> </td>
<td>|</td>
<td>
records: <%=strec%></td>
</tr>
