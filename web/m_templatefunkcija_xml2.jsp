...
var url="m_templatefunkcija_xml_add2.jsp";
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	   ...
            	   <td width=40% align=center bgcolor="D2EAFF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p1)">Ime</td>
            	...
            	<td width=30% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p15)">Class</td>
                 ...
                 <td  bgcolor="D2EAFF" align=center><a onClick="return godown2('m_templatefunkcijastart_edit2_xml.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>&glavno=1&idg=<%=request.getParameter("idg")%>')" href="Javascript: PGAPP()"><img title="Dodaj nov TemplateFunkcija" border="0" src="vnos.gif" /></a></td>
        </tr>
...
%>
	...
	<%
	if (vseb.getTemplatetag()!=null && vseb.getTemplatetag().getTag_name()!=null && nul.jeNull(vseb.getTemplatetag().getUporabi_tag()).equals("1")) {
		%>
		<b><i><%=nul.jeNull(vseb.getTemplatetag().getTag_name_parent())%></i> <%=nul.jeNull(vseb.getTemplatetag().getTag_name())%></b>
		<%
	}
	
	%>
	
	(<i><%=nul.jeNull(vseb.getIme_funkcije())%></i>)</td>
...
<td  align="center">
 ...
 <a href="Javascript:PGAPP()" onCLick="return go('<%=vseb.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=1',2)"><img src="t.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="XML Element"></a>
...
%>
 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein_xml2.jsp?idp=<%=vseb.getId()%>&classo=<%=nul.jeNull(vseb.getClass_name())%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=1')"><img src="c.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Pridobi podatek iz druge tabele"></a>
<%
//}
%>

			<a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_pogoj_framein_xml.jsp?idg=<%=vseb.getId()%>&opcija=spremeni&siframm=new&idtf=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>')">
<kimg src="pp.gif" alt="Pogoji prikaza" border="0"></a>




 <a onClick="return vprasaj()" href="m_templatefunkcija_xml2.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iu.hasNext()) {
	...
	<td  class="<%=colors[br]%>" width="70%" >
	
		<%
	if (tcv.getTemplatetag()!=null && tcv.getTemplatetag().getTag_name()!=null && nul.jeNull(tcv.getTemplatetag().getUporabi_tag()).equals("1")) {
		%>
		<b><i><%=nul.jeNull(tcv.getTemplatetag().getTag_name_parent())%></i> <%=nul.jeNull(tcv.getTemplatetag().getTag_name())%></b>
		<%
	}
	
	%>
	
	(<i><%=nul.jeNull(tcv.getVrednost())%></i>)
	</td>
		<td  class="<%=colors[br]%>" >
		...
		%>
	<a onClick="return godown('m_templatefunkcija_pogoj_edit_xml22.jsp?idv=<%=vseb.getId()%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
		 <a href="Javascript:PGAPP()" onCLick="return go('<%=tcv.getId()%>&opcija=spremeni&siframm=new&idback=<%=tcv.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=1',2)"><img src="t.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="XML Element"></a>
...
%>
			...
			<td width="30%" align="left"><%=nul.jeNull(si.getDodaten_hql_pogoj())%></td>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_pogoj_edit_xml22.jsp?idv=<%=vseb.getId()%>&id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 title="Spremeni podatke"></a>
		...
		%>
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein_xml2.jsp?idv=<%=vseb.getId()%>&idp=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>')"><img src="c.gif" border=0 title="Pridobi podatek iz druge tabele"></a>
		<%
		//}
		%>

			<a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_pogoj_framein_xml.jsp?idg=<%=si.getId()%>&opcija=spremeni&siframm=new&idtf=<%=si.getId()%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>')">
<imkg src="pp.gif" alt="Pogoji prikaza" border="0"></a>
		
		
		
		
		
		 <a onClick="return vprasaj()" href="m_templatefunkcija_xml2.jsp?akcija=DELETE&kaj=class&id=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatefunkcijastart_edit2_xml.jsp?id="+asd;
    parent.frames[1].location = aaa;
  }
  if (vv == 2){
    aaa = "m_templatefunkcijatag_xml.jsp?idv="+asd;
    parent.frames[1].location = aaa;
  }
return false;
}
function godown22(asd){
top.parent.frames[3].location = "m_template_xml_framein.jsp?loka="+asd;
return false;
}
...
<td align=center >
Browse page: </td><td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija_xml2.jsp?pagenumber=1<%=iskk%>">first</a></td><td>|</td>
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija_xml2.jsp?pagenumber=<%=prej%><%=iskk%>">previous</a></td>
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateFunkcija")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatefunkcija_xml2.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateFunkcija")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatefunkcija_xml2.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija_xml2.jsp?pagenumber=<%=prej%><%=iskk%>">next</a></td>
...
<td><a style="text-decoration:none;color:black"  href="m_templatefunkcija_xml2.jsp?pagenumber=<%=ststr+1%><%=iskk%>">last</a></td>
<td>|</td>
<td >
