...
<iframe noresize="noresize"  id="ifr2" name="prvain" target="drugain"  scrolling="yes" frameborder="0"   src="m_templateclass_pogoj.jsp?idt=<%=request.getParameter("idt") %>&idg=<%=request.getParameter("idg") %>&sifid=<%=request.getParameter("sifid") %>"   style="width:100%;height:100%"  >b</iframe>
        </div>
...
<iframe noresize="noresize" id="ifr3" name="drugain" target="prvain" src="nic.jsp" scrolling="yes" frameborder="0" style="width:100%;height:100%"  onload="collapseL(this.contentWindow.location)"  >b</iframe>
     ...
     $(document).ready(function () {
         ...
         $('#mainSplitter').jqxSplitter({ theme: theme, width: '100%', height: '100%', orientation: 'vertical', panels: [{ size: '35%',collapsible:false }, { size: '65%',collapsible:true}] });
