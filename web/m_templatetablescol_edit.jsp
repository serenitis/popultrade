...
ht.put(""+ConPool.getPrevod("18701J"+control.getJezik()).replaceAll("_"," "),"field_name");
ht.put(""+ConPool.getPrevod("18718J"+control.getJezik()).replaceAll("_"," "),"type");
ht.put(""+ConPool.getPrevod("18712J"+control.getJezik()).replaceAll("_"," "),"h_type_java");
ht.put(""+ConPool.getPrevod("18711J"+control.getJezik()).replaceAll("_"," "),"h_type");
...
%>
<form name=dod action="m_templatetablescol_edit.jsp" method="post"    onSubmit="return check()">
<%
...
<tr><td class="opiscell" width="20%" align="left">Ime polja hibernate (vedno z malo zacetnico):<br>Ime polja v bazi(ce razlicno od ime polja):<br>V primeru manytoone mora imeti isto ime kot povezovalna tabela (z malo zacetnico)</td><td class="valuecell"  width="30%" ><%
rezu=nul.jeNull(vseb.getField_name()+"");

%>
<input type="text"    tabindex='10' id="field_name"  name="field_name"     value="<%=rezu%>"    class="inputIEx"   onfocus="ajax_options_hide();change(this,'yellow');setKlici(this,1);"   onblur="change(this,'white');"  > za manytoone ime isto kot povezana tabela<br>
...
%>
<input type="text"    tabindex='22' id="width"  name="width"   size=3  value="<%=rezu%>" onkdeyup="(document.dod.width_decimals.value=='' || document.dod.width_decimals.value==document.dod.width.value )?document.dod.width_decimals.value=document.dod.width.value:''"   class="inputIEx"   onfocus="ajax_options_hide();change(this,'yellow');setKlici(this,1);"   onblur="change(this,'white');"  >
Sirina z decimalkami:
<% 
rezu=nul.jeNull(vseb.getWidth_decimals()+"");

%>
<input type="text"    tabindex='23' id="width_decimals"  name="width_decimals"   size=3  value="<%=rezu%>"    class="inputIEx"   onfocus="ajax_options_hide();change(this,'yellow');setKlici(this,1);"   onblur="change(this,'white');"  >
...
<tr><td class="opiscell" width="20%" align="left">Je primaren index:</td><td class="valuecell"  width="30%" >

<%
rezu=nul.jeNull(vseb.getIs_primarykey()+"");

%>

  <input type="checkbox" name="is_primarykey"  id="is_primarykey"  tabindex='24'  value="1" <%=nul.jeNull(rezu+"").equals("1")?"checked":""%> style="font-family: Verdana; font-size: 8pt">
	<b>V primeru NoSQL je id vedno String!!!!</b>
</td>
...
<tr><td class="opiscell" width="20%" align="left">Order by za relacije (tip povezave):</td><td class="valuecell"  width="30%" ><%
rezu=nul.jeNull(vseb.getOrderby_relations()+"");

%><input type='text' id="orderby_relations" name='orderby_relations'  class="inputIEx"   value="<%=rezu%>">
</td>
<td class="opiscell" ></td><td class="valuecell" ></td></tr>
...
<select id="h_connection_cascade"  name="h_connection_cascade"   tabindex='55'  class="inputIEx" >

<option value="REMOVE" <%=(rezu.equals("REMOVE")?"selected":"") %>>REMOVE</option>
<option value="ALL" <%=(rezu.equals("ALL")?"selected":"") %>>ALL</option>
<option value="NONE" <%=(rezu.equals("NONE")?"selected":"") %>>NONE</option>


</select>v primeru nosql veza na embeddable je ALL
...
<tr><td class="opiscell" width="20%" align="left">Fetch type:</td><td class="valuecell"  width="30%" ><%
rezu=nul.jeNull(vseb.getH_property_formula_name()+"");

%>

<select id="h_property_formula_name"  name="h_property_formula_name"   tabindex='55'  class="inputIEx" >
<option value="LAZY" <%=(rezu.equals("LAZY")?"selected":"") %>>LAZY</option>
<option value="EAGER" <%=(rezu.equals("EAGER")?"selected":"") %>>EAGER</option>




</select>
v primeru nosql veza na embeddable je EAGER
...
<tr><td class="opiscell" width="20%" align="left">Ime sekvence, ce je primary key (se ne rabi za NoSQL):</td><td class="valuecell"  width="30%" ><%
rezu=nul.jeNull(vseb.getSequence_name()+"");

%>
<input type="text"    tabindex='95' id="sequence_name"  name="sequence_name"     value="<%=rezu%>"    class="inputIEx"   onfocus="ajax_options_hide();change(this,'yellow');setKlici(this,1);"   onblur="change(this,'white');"  >
</td>
<td class="opiscell" width="20%" align="left">Tip vecanja sekvence oz increment type (ce ne gre za sekvenco):</td><td class="valuecell" width="30%">
<%
rezu=nul.jeNull(vseb.getIncrement_type()+"");

%>
<input type="text"    tabindex='100' id="increment_type"  name="increment_type"  readonly="readonly"   value="SEQUENCE"    class="inputIEx"   onfocus="ajax_options_hide();change(this,'yellow');setKlici(this,1);"   onblur="change(this,'white');"  >
za zdaj fiksno</td>
</tr>
<tr><td class="opiscell" width="20%" align="left" colspan=4>&nbsp;</td></tr>
<tr><td class="opiscell" width="20%" align="left">Formula (vpise se select ki vrne vrednost v polje (select mora vrnit le en rezultat ali noben)):</td><td colspan=3 class="valuecell"  width="30%" ><%
rezu=nul.jeNull(vseb.getH_property_formula()+"");

%>
<textarea tabindex='105' id="h_property_formula" cols=150 rows=2 name="h_property_formula"   style="font-family: Verdana; font-size: 8pt" clsass="inputIEx"   onfocus="ajax_options_hide();change(this,'yellow');setKlici(this,1);"   onblur="change(this,'white');"   ><%=rezu%></textarea>
...
function custom() {
	...
	if (document.dod.is_primarykey.checked) {
		
		if (trim(document.dod.sequence_name.value)=="") {

			alert("Napaka: ime sekvence ni vnesen!");
			return false;
		}
		
		if (trim(document.dod.increment_type.value)=="") {

			alert("Napaka: tip vecanja sekvence ni izbran!");
			return false;
		}
	...
	if (document.dod.h_connection_type.value=="") {
		...
		if (document.dod.h_type_java.value=="") {
			alert("Napaka: tip polja v javi ni izbrano!");
			return false;
		}
...
if (mon.indexOf("id=")!=-1) {
	...
	<%=parent%>.location = parent.frames[2].location + "&idback=<%=request.getParameter("id")%>";
}
}
else {
	<%=parent%>.location =parent.frames[2].location + "?idback=<%=request.getParameter("id")%>";
...
if (mon.indexOf("DELETE")!=-1) {
	...
	<%=parent%>.location = parent.frames[2].location;
}
<%
}
%>
