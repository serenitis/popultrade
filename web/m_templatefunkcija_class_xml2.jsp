...
document.location = "m_templatefunkcija_class_xml2.jsp?kaj=insert&idm=<%=sif.getId()%>&sifid=<%=sifid%>&tip_funkcije="+document.getElementById("tip_funkcije").value+"&class="+document.getElementById("class_name").value;
...
while (iter.hasNext()) {
	...
	<td>
	 ...
	 <a onClick="return vprasaj()" href="m_templatefunkcija_class_xml2.jsp?idp=<%=request.getParameter("idp")%>&akcija=DELETE&idd=<%=temc.getId()%>&sifid=<%=sifid%>&kaj=pogoji"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iter.hasNext()) {
	...
	<%=nul.jeNull(temc.getVrednost())%> (<%=nul.jeNull(temc.getRabi_kot())%>)</td><td>
	 ...
	 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_vrednosti_xml2.jsp?idp=<%=request.getParameter("idp")%>&id=<%=temc.getId()%>&idtc=<%=sif.getId()%>&sifid=<%=sifid%>&opcija=spremeni&siframm=new&idback=<%=temc.getId()%>')"><img src="u.gif" border=0 title="Spremeni podatke"></a>
	 <a onClick="return vprasaj()" href="m_templatefunkcija_class_xml2.jsp?idp=<%=request.getParameter("idp")%>&akcija=DELETE&idd=<%=temc.getId()%>&sifid=<%=sifid%>&kaj=vrednosti"><img src="x.gif" border=0 title="Brisi"></a>
...
<a onClick="return godown('m_templatefunkcija_class_vrednosti_xml2.jsp?idp=<%=request.getParameter("idp")%>&opcija=new&siframm=new&sifrant=null&sifid=<%=sifid%>&idtc=<%=sif.getId()%>')" href="Javascript: PGAPP()"><img title="Dodaj novo vrednost" border="0" src="vnos.gif" /></a>
