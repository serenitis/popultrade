...
while (itep.hasNext()) {
	...
	prep.put(prv.getPrevod_id()+"",(prv.getPrevod()+"").trim());
}
...
while (itep.hasNext()) {
	com.popultrade.model.Prevodi prv = (com.popultrade.model.Prevodi)itep.next();
	prev.put(prv.getPrevod_id()+"",(prv.getPrevod()+"").trim());
...
while (itep.hasNext()) {
	com.popultrade.model.Prevodi prv = (com.popultrade.model.Prevodi)itep.next();
	prevh.put(prv.getPrevod_id()+"",(prv.getPrevod()+"").trim());
}
...
while (itep.hasNext()) {
	com.popultrade.model.Prevodi prv = (com.popultrade.model.Prevodi)itep.next();
	preph.put(prv.getPrevod_id()+"",(prv.getPrevod()+"").trim());
}
...
while (itep.hasNext()) {
	com.popultrade.model.Prevodi prv = (com.popultrade.model.Prevodi)itep.next();
	prerv.put(prv.getPrevod_id()+"",(prv.getPrevod()+"").trim());
}
...
while (itep.hasNext()) {
	com.popultrade.model.Prevodi prv = (com.popultrade.model.Prevodi)itep.next();
	prerh.put(prv.getPrevod_id()+"",(prv.getPrevod()+"").trim());
}
*/
/*


com.popultrade.dao.PrevodiDAO daol = (com.popultrade.dao.PrevodiDAO)contextUtil.getBeanDao("prevodiDAO",pageContext.getServletContext());

List lio = daol.getPrevodisByJezik(session.getAttribute("jezikv")+"");


Iterator itm = lio.iterator();

//	System.out.println("prevod size: "+lio.size());
Hashtable prevodi = new Hashtable();
Hashtable prevodip = new Hashtable();
while (itm.hasNext()) {
	
	com.popultrade.model.Prevodi prevk = (com.popultrade.model.Prevodi)itm.next();
//	System.out.println("PREV: "+prev.getPrevod_id()+prev.getVrsta()+prev.getSifra_jez()+" ::: "+prev.getPrevod()==null?"":prev.getPrevod() );
	prevodi.put(prevk.getPrevod_id()+prevk.getVrsta()+prevk.getSifra_jez(), prevk.getPrevod()==null?"":prevk.getPrevod());
	previd.put(prevk.getPrevod_id()+"",prevk.getId()+"");
}

lio.clear();
 lio = daol.getPrevodisByJezik(session.getAttribute("jezikp")+"");
 itm = lio.iterator();
while (itm.hasNext()) {
	com.popultrade.model.Prevodi prevk = (com.popultrade.model.Prevodi)itm.next();
//	System.out.println("PREV: "+prev.getPrevod_id()+prev.getVrsta()+prev.getSifra_jez()+" ::: "+prev.getPrevod()==null?"":prev.getPrevod() );
	prevodip.put(prevk.getPrevod_id()+prevk.getVrsta()+prevk.getSifra_jez(), prevk.getPrevod()==null?"":prevk.getPrevod());
...
<form name="dod" action="m_templatelist_t.jsp?idt=<%=request.getParameter("idt")%>&akcija=prevod" method="post">
...
%>
	...
	<b>
	<%=nul.jeNull(vseb.getStolpec_naziv())%></b><%=nul.jeNull(vseb.getFormat_stolpca()).equals("")?"":"<br>F: "+vseb.getFormat_stolpca()%>
	</td></tr>
		<tr><td class="valuecell" >
<%=nul.jeNull(ConPool.getPrevod_admin(vseb.getId()+"","H",session.getAttribute("jezikp")+""))%><br>
Hint:<input type="text" name="prevod_<%=vseb.getId()%>_H_<%=nul.jeNull(ConPool.getPrevod_admin_id(vseb.getId()+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(vseb.getId()+"","H",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">
<br>&nbsp;<br> 
<%=nul.jeNull(ConPool.getPrevod_admin(vseb.getId()+"","P",session.getAttribute("jezikp")+""))%><br>
Prevod:<input type="text" name="prevod_<%=vseb.getId()%>_P_<%=nul.jeNull(ConPool.getPrevod_admin_id(vseb.getId()+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(vseb.getId()+"","P",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">
	</td></tr>
	<tr><td class="valuecell" >
	<b>
	...
	Z: <%=nul.jeNull(vseb.getZaporedje()+"")%>
		...
		<tr><td class="valuecell" >
	<a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_pogoj_framein.jsp?idg=<%=vseb.getId()%>&opcija=spremeni&siframm=new&idtf=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>')">
	...
	<tr><td class="valuecell" >
	 ...
	 <br><a onClick="return vprasaj()" href="m_templatelist_t.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
<input type="text" name="prevod_<%=request.getParameter("idt")%>_R_<%=nul.jeNull(ConPool.getPrevod_admin_id(request.getParameter("idt")+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","R",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">(potrdi brisanje)
  ...
  <a onClick="return godown('m_templatelist_edit_t.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>')" href="Javascript: PGAPP()"><img title="Dodaj novi stolpec" border="0" src="vnos.gif" /></a>
  <br>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatelist_edit_t.jsp?id="+asd;
...
<td align=center >
Browse page: </td><td>
 <a style="text-decoration:none;color:black"  href="m_templatelist_t.jsp?pagenumber=1<%=iskk%>">first</a></td><td>|</td>
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatelist_t.jsp?pagenumber=<%=prej%><%=iskk%>">previous</a></td>
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateList")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatelist_t.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateList")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatelist_t.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatelist_t.jsp?pagenumber=<%=prej%><%=iskk%>">next</a></td>
...
<td><a style="text-decoration:none;color:black"  href="m_templatelist_t.jsp?pagenumber=<%=ststr+1%><%=iskk%>">last</a></td>
<td>|</td>
<td >
