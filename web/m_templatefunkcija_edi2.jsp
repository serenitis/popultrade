...
var url="m_templatefunkcija_edi_add2.jsp";
...
<td  align="center">
 ...
 <a href="Javascript:PGAPP()" onCLick="return go('<%=vseb.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=1',1)"><img src="u.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Spremeni podatke"></a>
		...
		<%
		 if (vseb.getTemplateClassClass()!=null && vseb.getTemplateClassClass().getId()!=null ) {
			%>
			
			 <a href="Javascript:PGAPP()" onCLick="return godown22('m_templatelist_edi.jspQUEidt=<%=vseb.getId()%>ANDidnn=<%=vseb.getTemplateClassClass().getId()%>ANDclassname=<%=vseb.getTemplateClassClass().getClass_name()%>')"><img src="p1.gif" border=0 title="Spremeni nacin prikaza"></a>
...
%>
 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein_edi2.jsp?idp=<%=vseb.getId()%>&classo=<%=nul.jeNull(vseb.getClass_name())%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=1')"><img src="c.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Pridobi podatek iz druge tabele"></a>
<%
//}
%>
 <a onClick="return vprasaj()" href="m_templatefunkcija_edi2.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iu.hasNext()) {
	...
	<td  class="<%=colors[br]%>" width="70%" ><%=nul.jeNull(tcv.getVrednost())%>
		...
		%>
	<a onClick="return godown('m_templatefunkcija_pogoj_edit_edi22.jsp?idv=<%=vseb.getId()%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
		<%
		}
		%>
...
%>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_pogoj_edit_edi22.jsp?idv=<%=vseb.getId()%>&id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 title="Spremeni podatke"></a>
		
		<%
	 if (si.getTemplateClassClass()!=null && si.getTemplateClassClass().getId()!=null ) {
			%>
			
			 <a href="Javascript:PGAPP()" onCLick="return godown22('m_templatelist_edi.jspQUEidt=<%=si.getTemplateClassClass().getId()%>ANDidnn=<%=vseb.getId()%>ANDclassname=<%=si.getTemplateClassClass().getClass_name()%>')"><img src="p1.gif" border=0 title="Spremeni nacin prikaza"></a>
		...
		%>
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein_edi2.jsp?idv=<%=vseb.getId()%>&idp=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>')"><img src="c.gif" border=0 title="Pridobi podatek iz druge tabele"></a>
		<%
		//}
		%>
...
top.parent.frames[3].location = "m_template_edi_framein.jsp?loka="+asd;
return false;
}
...
<td align=center >
Browse page: </td><td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija_edi2.jsp?pagenumber=1<%=iskk%>">first</a></td><td>|</td>
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija_edi2.jsp?pagenumber=<%=prej%><%=iskk%>">previous</a></td>
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateFunkcija")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatefunkcija_edi2.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateFunkcija")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatefunkcija_edi2.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija_edi2.jsp?pagenumber=<%=prej%><%=iskk%>">next</a></td>
...
<td><a style="text-decoration:none;color:black"  href="m_templatefunkcija_edi2.jsp?pagenumber=<%=ststr+1%><%=iskk%>">last</a></td>
<td>|</td>
<td >
