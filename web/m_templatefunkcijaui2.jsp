...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	...
            	<td width=30% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p15)">Class</td>
                 ...
                 <td  bgcolor="D2EAFF" align=center><a onClick="return godown2('m_templatefunkcijastartui_edit2.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>&glavno=1&idg=<%=request.getParameter("idg")%>')" href="Javascript: PGAPP()"><img title="Dodaj nov TemplateFunkcija" border="0" src="vnos.gif" /></a></td>
        </tr>
...
%>
 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcijaui_class_framein2.jsp?idp=<%=vseb.getId()%>&classo=<%=nul.jeNull(vseb.getClass_name())%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=1')"><img src="c.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Pridobi podatek iz druge tabele"></a>
<%
//}
%>
 <a onClick="return vprasaj()" href="m_templatefunkcijaui2.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iu.hasNext()) {
	...
	<td  class="<%=colors[br]%>" width="70%" ><%=nul.jeNull(tcv.getVrednost())%>
		...
		%>
	<a onClick="return godown('m_templatefunkcijaui_pogoj_edit22.jsp?idv=<%=vseb.getId()%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
		<%
		}
		%>
...
%>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcijaui_pogoj_edit22.jsp?idv=<%=vseb.getId()%>&id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 title="Spremeni podatke"></a>
		<%
		//if (nul.jeNull(vseb.getPredpona_pogoja()).equals("VALUE")) {

		%>
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcijaui_class_framein2.jsp?idv=<%=vseb.getId()%>&idp=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>')"><img src="c.gif" border=0 title="Pridobi podatek iz druge tabele"></a>
		<%
		//}
		%>
		 <a onClick="return vprasaj()" href="m_templatefunkcijaui2.jsp?akcija=DELETE&kaj=class&id=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatefunkcijastartui_edit2.jsp?id="+asd;
