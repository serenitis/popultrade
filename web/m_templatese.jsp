...
<form name="dod" action="m_templatese.jsp?idt=<%=request.getParameter("idt")%>&akcija=prevod" method="post">
...
<input type="text" name="prevod_<%=vsebb.getId()%>_P_<%=nul.jeNull((String)ConPool.getPrevod_admin_id(vsebb.getId()+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(vsebb.getId()+"","P",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">
	...
	<%
	if (nul.jeNull(vsebb.getKontrola_javascript()).equals("DA") || nul.jeNull(vsebb.getDodatna_js_kontrola_uporabi()).equals("1")) {
		%><br><font color=red>Kontrola JS:</font><br>
			<%=nul.jeNull(ConPool.getPrevod_admin(vseb.getId()+"","J",session.getAttribute("jezikp")+""))%><br>
<input type="text" name="prevod_<%=vsebb.getId()%>_J_<%=nul.jeNull((String)ConPool.getPrevod_admin_id(vsebb.getId()+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(vseb.getId()+"","J",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">
  ...
  <tr><td width=50%>
	...
	<a onClick="return vprasaj()" href="m_templatese.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
%>
	...
	<a onClick="return vprasaj()" href="m_templatese.jsp?akcija=DELETE&id=<%=vsebb.getId()%>&idt=<%=request.getParameter("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
%>
	<tr><td colspan=7 ><a onClick="return godown('m_templatese_edit.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>&idn=<%=vseb.getId()%>')" href="Javascript: PGAPP()"><img title="Dodaj v isto polje" border="0" src="next.gif" /></a></td></tr>
	</table>
	</td>
...
<tr>
	...
	Z: <%=nul.jeNull(vseb.getZaporedje()+"")%>
	 ...
	 <a href="Javascript:PGAPP()" onCLick="return go('<%=vseb.getId()%>&opcija=spremeni&siframm=new&idtf=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>',1)"><img src="u.gif" border=0  title="Spremeni podatke"></a>
	 <br><a onClick="return vprasaj()" href="m_templatese.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
<td class="opiscell" width="20%">&nbsp;</td><td class="valuecell" width="30%">
	<a onClick="return godown('m_templatese_edit.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>')" href="Javascript: PGAPP()"><img title="Dodaj novo polje" border="0" src="vnos.gif" /></a> &nbsp;
...
if (lis.size()>0) {
	...
	<tr><td>
	 	...
	 	<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","S",session.getAttribute("jezikp")+""))%>
<input type="text" name="prevod_<%=request.getParameter("idt")%>_S_<%=nul.jeNull((String)ConPool.getPrevod_admin_id(request.getParameter("idt")+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","S",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">(reset)
</td><td>
	 	<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","R",session.getAttribute("jezikp")+""))%>
<input type="text" name="prevod_<%=request.getParameter("idt")%>_R_<%=nul.jeNull((String)ConPool.getPrevod_admin_id(request.getParameter("idt")+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","R",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">(potrdi)

</td>

<td>Odgovor ob uspesnem insertu ali update:<br>
	 	<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","X",session.getAttribute("jezikp")+""))%>
<input type="text" name="prevod_<%=request.getParameter("idt")%>_X_<%=nul.jeNull((String)ConPool.getPrevod_admin_id(request.getParameter("idt")+""))%>" value="<%=nul.jeNull(ConPool.getPrevod_admin(request.getParameter("idt")+"","X",session.getAttribute("jezikv")+""))%>" style="font-family: Verdana; font-size: 8pt">(potrdi)
 ...
 <tr><td colspan=4 align=center bgcolor=silver height=32>
  ...
  <input type="button" value="Dodatni pogoji"  style="font-family: Verdana; font-size: 8pt" onClick="return godown('m_templateuidodatno_edit_search.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>')">
