...
<form name="dod" action="m_templatelist_edi.jsp?idt=<%=request.getParameter("idt")%>&akcija=prevod" method="post">
...
%>
	...
	Z: <%=nul.jeNull(vseb.getZaporedje()+"")%>
		...
		<tr><td class="valuecell" >
	<a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_pogoj_edi_framein.jsp?idg=<%=vseb.getId()%>&opcija=spremeni&siframm=new&idtf=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>')">
	...
	<tr><td class="valuecell" >
	 ...
	 <br><a onClick="return vprasaj()" href="m_templatelist_edi.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idt=<%=request.getParameter("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
br++;
 ...
 %>
  ...
  <a onClick="return godown('m_templatelist_edit_edi.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>')" href="Javascript: PGAPP()"><img title="Dodaj novi stolpec" border="0" src="vnos.gif" /></a>
  <br>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatelist_edit_edi.jsp?id="+asd;
...
<td align=center >
Browse page: </td><td>
 <a style="text-decoration:none;color:black"  href="m_templatelist_edi.jsp?pagenumber=1<%=iskk%>">first</a></td><td>|</td>
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatelist_edi.jsp?pagenumber=<%=prej%><%=iskk%>">previous</a></td>
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateList")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatelist_edi.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateList")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatelist_edi.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatelist_edi.jsp?pagenumber=<%=prej%><%=iskk%>">next</a></td>
...
<td><a style="text-decoration:none;color:black"  href="m_templatelist_edi.jsp?pagenumber=<%=ststr+1%><%=iskk%>">last</a></td>
<td>|</td>
<td >
