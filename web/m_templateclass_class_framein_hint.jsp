...
<iframe noresize="noresize"  id="ifr2" name="prvain" target="drugain"  scrolling="yes" frameborder="0"   src="m_templateclass_class_hint.jsp?idt=<%=request.getParameter("idt") %>&sifid=<%=request.getParameter("sifid") %>&idp=<%=request.getParameter("idp") %>&idg=<%=request.getParameter("idg") %>&glavno=<%=request.getParameter("glavno") %>"   style="width:100%;height:100%"  >b</iframe>
        </div>
