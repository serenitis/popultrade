...
while (iu.hasNext()) {
	...
	(<i><%=nul.jeNull(tcv.getVrednost())%></i>)
		...
		%>
	<a onClick="return godown('m_templatefunkcija_pogoj_edit_xml22.jsp?idv=<%=request.getParameter("idgumb")%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
		 <a href="Javascript:PGAPP()" onCLick="return go('<%=tcv.getId()%>&opcija=spremeni&siframm=new&idback=<%=tcv.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=1',2)"><img src="t.gif" border=0  title="XML Element"></a>
...
%>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_pogoj_edit_xml22.jsp?idv=<%=request.getParameter("idgumb")%>&id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=request.getParameter("idgumb")%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 title="Spremeni podatke"></a>
		...
		%>
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein_xml2.jsp?idv=<%=request.getParameter("idgumb")%>&idp=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>')"><img src="c.gif" border=0  title="Pridobi podatek iz druge tabele"></a>
		<%
		//}
		%>
					...
					<a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_pogoj_framein_xml.jsp?idg=<%=si.getId()%>&opcija=spremeni&siframm=new&idtf=<%=si.getId()%>&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>')">
	<imsg src="pp.gif" alt="Pogoji prikaza" border="0">
	</a>
