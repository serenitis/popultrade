...
while (iu.hasNext()) {
	...
	<td   width="70%" ><%=nul.jeNull(tcv.getVrednost())%>
		...
		%>
	<a onClick="return godown('m_templatefunkcija_pogoj_edit_pdf22.jsp?idv=<%=request.getParameter("idgumb")%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
...
%>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_pogoj_edit_pdf22.jsp?idv=<%=request.getParameter("idgumb")%>&id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=request.getParameter("idgumb")%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 title="Spremeni podatke"></a>
		
		<%
		if (si.getTemplateClassClass()!=null && si.getTemplateClassClass().getId()!=null && nul.jeNull(si.getTemplateClassClass().getTip_funkcije()).equals("ER")) {
		
		%>
		
		 <a href="Javascript:PGAPP()" onCLick="return godown22('m_templateview_pdf.jspQUEidt=<%=si.getTemplateClassClass().getId()%>ANDidnn=<%=request.getParameter("idgumb")%>ANDclassname=<%=si.getTemplateClassClass().getClass_name()%>')"><img src="i.gif" border=0 title="Spremeni nacin prikaza"></a>
		
		<%
		}
		else if (si.getTemplateClassClass()!=null && si.getTemplateClassClass().getId()!=null && nul.jeNull(si.getTemplateClassClass().getTip_funkcije()).equals("LI")) {
			%>
			
			 <a href="Javascript:PGAPP()" onCLick="return godown22('m_templatelist_pdf.jspQUEidt=<%=si.getTemplateClassClass().getId()%>ANDidnn=<%=request.getParameter("idgumb")%>ANDclassname=<%=si.getTemplateClassClass().getClass_name()%>')"><img src="p1.gif" border=0 title="Spremeni nacin prikaza"></a>
			
			<%
			
		}
		else if (si.getTemplateClassClass()!=null && si.getTemplateClassClass().getId()!=null && nul.jeNull(si.getTemplateClassClass().getTip_funkcije()).equals("CO")) {
			%>
			
			 <a href="Javascript:PGAPP()" onCLick="return godown22('m_templatelist_pdf.jspQUEidt=<%=si.getTemplateClassClass().getId()%>ANDidnn=<%=request.getParameter("idgumb")%>ANDclassname=<%=si.getTemplateClassClass().getClass_name()%>')"><img src="p1.gif" border=0 title="Spremeni nacin prikaza list"></a>
				 <a href="Javascript:PGAPP()" onCLick="return godown22('m_templateview_pdf.jspQUEidt=<%=si.getTemplateClassClass().getId()%>ANDidnn=<%=request.getParameter("idgumb")%>ANDclassname=<%=si.getTemplateClassClass().getClass_name()%>')"><img src="i.gif" border=0 title="Spremeni nacin prikaza view"></a>
		...
		%>
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein_pdf2.jsp?idv=<%=request.getParameter("idgumb")%>&idp=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>')"><img src="c.gif" border=0  title="Pridobi podatek iz druge tabele"></a>
		<%
		//}
		%>
