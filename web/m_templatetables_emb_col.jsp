...
sif.setId_nad(new Long(request.getParameter("idt")));
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
 ...
 <td width=25% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()"  ondragover="allowDrop(event)"  ondrop="drop('TemplateTablesCol','')"  onmousemove="ddrivetip(p1)">Ime polja</td>
                 ...
                 <td width=38% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p9)">Description</td>
                 ...
                 <a onClick="return godown('m_templatetablescol_emb_edit.jsp?opcija=new&siframm=new&sifrant=null&idg=<%=request.getParameter("idg") %>&idt=<%=request.getParameter("idt") %>&id_nad=<%=request.getParameter("id_nad") %>')" href="Javascript: PGAPP()"><img title="Dodaj novo polje" border="0" src="vnos.gif" /></a>
...
%>
<td width="25%" align="left" draggable="true"  ondragover="allowDrop(event)"  ondrop="drop('TemplateGumbi','&_Ltemplatehead_id=<%=vseb.getId() %>')"  ondragstart="dragStart(event,'TemplateTablesCol','tem1','&id=<%=vseb.getId()%>')" >
...
<td width="38%" align="left"><%=nul.jeNull(vseb.getDescription())%></td>

<td  align="center">

 <a href="Javascript:PGAPP()" onCLick="return go('<%=vseb.getId()%>&id_nad=<%=request.getParameter("id_nad") %>&idt=<%=request.getParameter("idt") %>&idg=<%=request.getParameter("idg") %>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&sifrant=<%=request.getParameter("sifrant")%>',1)"><img src="u.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Spremeni podatke"></a>


 <a onClick="vprasaj()?document.location='m_templatetables_emb_col.jsp?akcija=DELETE&idt=<%=request.getParameter("idt") %>&idg=<%=request.getParameter("idg") %>&id_nad=<%=request.getParameter("id_nad") %>&id=<%=vseb.getId()%>&sifrant=<%=request.getParameter("sifrant")%>':''" href='#'><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatetablescol_emb_edit.jsp?id="+asd;
