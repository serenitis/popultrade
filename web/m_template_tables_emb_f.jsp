...
<iframe noresize="noresize"  id="ifr2" name="prvain" target="drugain"  scrolling="yes" frameborder="0"   src="m_template_tables_emb.jsp?id_nad=<%=request.getParameter("id_nad") %>&sifid=<%=request.getParameter("sifid") %>&idg=<%=request.getParameter("idg") %>&idg=<%=request.getParameter("idg") %>&glavno=<%=request.getParameter("glavno")%>&stopnja=<%=request.getParameter("stopnja") %>"   style="width:100%;height:100%"  >b</iframe>
        </div>
...
<iframe noresize="noresize" id="ifr3" name="drugain" target="prvain" src="nic.jsp" scrolling="yes" frameborder="0" style="width:100%;height:100%"  onload="collapseL(this.contentWindow.location)"  >b</iframe>
     ...
     $(document).ready(function () {
         ...
         $('#mainSplitter').jqxSplitter({ theme: theme, width: '100%', height: '100%', orientation: 'vertical', panels: [{ size: '40%',collapsible:false }, { size: '60%',collapsible:true}] });
