...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
 ...
 <td width=30% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()"  ondragover="allowDrop(event)"  ondrop="drop('TemplateTables','')"  onmousemove="ddrivetip(p1)">Table name</td>

            	   <td width=30% align=center bgcolor="D2EAFF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p3)">Hibernate table name</td>
               
            	   
         	   <td width=20% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p4)">LPD</td>       	
               
            	    <td width=6% align=center bgcolor="D2EAFF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p6)">NoSQL</td>       	
                 ...
                 <a onClick="return godown('m_templatetables_edit.jsp?opcija=new&siframm=new&sifrant=null&idg=<%=request.getParameter("idg") %>')" href="Javascript: PGAPP()"><img title="Dodaj novo tabelo" border="0" src="vnos.gif" /></a>
...
%>
<td width="30%" align="left" draggable="true"  ondragover="allowDrop(event)"  ondrop="drop('TemplateGumbi','&_Ltemplatehead_id=<%=vseb.getId() %>')"  ondragstart="dragStart(event,'TemplateTables','tem1','&id=<%=vseb.getId()%>')" >



<%=nul.jeNull(vseb.getTable_name())%></td>
	<td width="30%" align="left"><%=nul.jeNull(vseb.getTable_name_hibernate())%></td>

<td width="20%" align="center"><%=nul.getDatumFormatFromDate(vseb.getLast_build(),"dd-MM-yyyy HH:mm:ss")%></td>
<td width="6%" align="center"><%=nul.jeN(vseb.getTable_type())%></td>
...
<a href="Javascript:PGAPP()" onCLick="return godown('m_templatetables_col.jsp?id_nad=<%=vseb.getId()%>&idg=<%=request.getParameter("idg") %>&opcija=spremeni&siframm=new&sifrant=<%=request.getParameter("sifrant")%>',1)"><img src="i.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Polja v tabeli"></a>
<a href="Javascript:PGAPP()" onCLick="return godown('m_templategen_copy_list_p.jsp?modelname=<%=nul.jeNull(vseb.getTable_name_hibernate())%>&idg=<%=request.getParameter("idg") %>',1)"><img src="l.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Prikaz vezav z drugimi tabelami"></a>
<a href="Javascript:PGAPP()" onCLick="return godown('m_templatetableconf_framein.jsp?id_nad=<%=vseb.getId()%>&tablename=<%=nul.jeNull(vseb.getTable_name())%>&modelname=<%=nul.jeNull(vseb.getTable_name_hibernate())%>&idg=<%=request.getParameter("idg") %>',1)"><img src="c.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Primerjaj hibernate class s tabelo v bazi"></a>


<%
/// embedable classe za nosql
if (nul.jeN(vseb.getTable_type()).equals("1")) {
%>
<a href="Javascript:PGAPP()" onCLick="return godown('m_template_tables_emb_f.jsp?id_nad=<%=vseb.getId()%>&idg=<%=request.getParameter("idg") %>&opcija=spremeni&siframm=new&sifrant=<%=request.getParameter("sifrant")%>&tip=EMBEDABLE',1)"><img src="e.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Embedable classe"></a>

<%
}
%>

 <a onClick="vprasaj()?document.location='m_template_tables.jsp?akcija=DELETE&idg=<%=request.getParameter("idg") %>&id=<%=vseb.getId()%>&sifrant=<%=request.getParameter("sifrant")%>':''" href='#'><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatetables_edit.jsp?id="+asd;
