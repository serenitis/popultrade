...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	...
            	<td width=60% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p7)">Vrednost</td>
            	
        

              
                 <td  bgcolor="D2EAFF" align=center><a onClick="return godown('m_templateddp_edit.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>&glavno=1&idg=<%=request.getParameter("idg")%>')" href="Javascript: PGAPP()"><img title="Dodaj nov TemplateDDPogoji" border="0" src="vnos.gif" /></a></td>
        </tr>
...
%>
	...
	<td width="60%" align="left"><%=nul.jeNull(vseb.getVrednost_metode())%></td>
...
<td  align="center">
 ...
 <a onClick="return vprasaj()" href="m_templateddp.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templateddp_edit.jsp?id="+asd;
