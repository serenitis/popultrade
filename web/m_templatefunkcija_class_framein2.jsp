...
<html>
 ...
 <frameset rows="50%,50%" framespacing="0" border="<%=size%>" frameborder="<%=size%>">
  <frame name="prvain" target="drugain"  marginheight="1"  scrolling="<%=scrol %>"   src="m_templatefunkcija_class2.jsp?idt=<%=request.getParameter("idt") %>&sifid=<%=request.getParameter("sifid") %>&idp=<%=request.getParameter("idp") %>&idg=<%=request.getParameter("idg") %>&glavno=<%=request.getParameter("glavno") %>"   style="border-style: solid; border-width: 1">
