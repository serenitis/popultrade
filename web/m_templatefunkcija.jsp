...
var url="m_templatefunkcija_add.jsp";
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="CENTER" valign="MIDDLE" style=" font-family:Verdana; font-size:7pt">
        <tr bgcolor="2686D8">
            	...
            	<td width=30% align=center bgcolor="ADD9FF" onmouseout="hideddrivetip()" onmousemove="ddrivetip(p15)">Class</td>
                 ...
                 <td  bgcolor="D2EAFF" align=center><a onClick="return godown2('m_templatefunkcijastart_edit.jsp?opcija=new&siframm=new&sifrant=null&idt=<%=request.getParameter("idt")%>&sifid=<%=request.getParameter("idt")%>&glavno=1&idg=<%=request.getParameter("idg")%>')" href="Javascript: PGAPP()"><img title="Dodaj nov TemplateFunkcija" border="0" src="vnos.gif" /></a></td>
        </tr>
...
%>
 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein.jsp?idp=<%=vseb.getId()%>&classo=<%=nul.jeNull(vseb.getClass_name())%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=1')"><img src="c.gif" border=0 onClick="kliknivSivo(this,'<%=colors[br]%>');" title="Pridobi podatek iz druge tabele"></a>
<%
//}
%>
 <a onClick="return vprasaj()" href="m_templatefunkcija.jsp?akcija=DELETE&id=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iu.hasNext()) {
	...
	<td  class="<%=colors[br]%>" width="70%" ><%=nul.jeNull(tcv.getVrednost())%>
		...
		%>
	<a onClick="return godown('m_templatefunkcija_pogoj_edit2.jsp?idv=<%=vseb.getId()%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
		<%
		}
		%>
...
%>
		...
		<td  align="center">
		 ...
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_pogoj_edit2.jsp?idv=<%=vseb.getId()%>&id=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=vseb.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>&sifid=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>',1)"><img src="u.gif" border=0 title="Spremeni podatke"></a>
		<%
		//if (nul.jeNull(vseb.getPredpona_pogoja()).equals("VALUE")) {

		%>
		 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_framein.jsp?idv=<%=vseb.getId()%>&idp=<%=si.getId()%>&opcija=spremeni&siframm=new&idback=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=session.getAttribute("idt")%>&idt=<%=session.getAttribute("idt")%>&glavno=2&vrednost=<%=tcv.getVrednost()%>')"><img src="c.gif" border=0 title="Pridobi podatek iz druge tabele"></a>
		<%
		//}
		%>
		 <a onClick="return vprasaj()" href="m_templatefunkcija.jsp?akcija=DELETE&kaj=class&id=<%=si.getId()%>&idg=<%=session.getAttribute("idg")%>&idt=<%=session.getAttribute("idt")%>"><img src="x.gif" border=0 title="Brisi"></a>
...
function go(asd,vv){
  if (vv == 1){
    aaa = "m_templatefunkcijastart_edit.jsp?id="+asd;
...
<td align=center >
Browse page: </td><td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija.jsp?pagenumber=1<%=iskk%>">first</a></td><td>|</td>
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija.jsp?pagenumber=<%=prej%><%=iskk%>">previous</a></td>
...
%>
<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateFunkcija")+"").equals(ststr+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatefunkcija.jsp?pagenumber=<%=ststr %><%=iskk%>"><%=ststr %></a> </td><td>|</td>
<%
...
if (vse!=0) {
	%>
	<td align=center bgcolor="<%=nul.jeNull(session.getValue("pagenumberTemplateFunkcija")+"").equals(ststr+1+"")?"white":"ededed" %>"> <a hrefs="" style="text-decoration:none;color:black"  href="m_templatefunkcija.jsp?pagenumber=<%=ststr+1 %><%=iskk%>"><%=ststr+1 %></a> </td><td>|</td>
	<%
...
%>
<td>
 <a style="text-decoration:none;color:black"  href="m_templatefunkcija.jsp?pagenumber=<%=prej%><%=iskk%>">next</a></td>
...
<td><a style="text-decoration:none;color:black"  href="m_templatefunkcija.jsp?pagenumber=<%=ststr+1%><%=iskk%>">last</a></td>
<td>|</td>
<td >
