...
try {
	...
	if (request.getParameter("kaj")!=null && request.getParameter("kaj").equals("pogoji"))  {
	...
	else if (request.getParameter("kaj")!=null && request.getParameter("kaj").equals("isto"))  {
		...
		dao.removeTemplateClassClass(new Long(request.getParameter("idd")));
...
document.location = "m_templateclass_class_hint.jsp?kaj=insert&idm=<%=sif.getId()%>&sifid=<%=sifid%>&class="+document.getElementById("class_name").value;
...
<tr   class="<%=colors[0]%>" >
	...
	<td width="30%" align="left">
					...
					<input type="button" value="Dodatni pogoji select"  style="font-family: Verdana; font-size: 8pt" onClick="godown('m_templateclassclass_pdf.jsp?id=<%=nul.jeNull(sif.getId()+"")%>')" > (uporabi se samo order by)
 ...
 <a onClick="return vprasaj()" href="m_templateclass_class_hint.jsp?idp=<%=request.getParameter("idp")%>&akcija=DELETE&idd=<%=sif.getId()%>&sifid=<%=sifid%>&kaj=isto"><img src="x.gif" border=0 title="Odstrani classo"></a>
...
while (iter.hasNext()) {
	...
	<td>
	 ...
	 <a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_edit2.jsp?idp=<%=session.getAttribute("idp")%>&id=<%=temc.getId()%>&idtc=<%=sif.getId()%>&sifid=<%=sifid%>&opcija=spremeni&siframm=new&idback=<%=temc.getId()%>')"><img src="u.gif" border=0  title="Spremeni podatke"></a>
	 <a onClick="return vprasaj()" href="m_templateclass_class_hint.jsp?idp=<%=session.getAttribute("idp")%>&akcija=DELETE&idd=<%=temc.getId()%>&sifid=<%=sifid%>&kaj=pogoji"><img src="x.gif" border=0 title="Brisi"></a>
...
<a onClick="return godown('m_templateclass_edit2.jsp?idp=<%=session.getAttribute("idp")%>&opcija=new&siframm=new&sifrant=null&sifid=<%=sifid%>&idtc=<%=sif.getId()%>')" href="Javascript: PGAPP()"><img title="Dodaj novi pogoj" border="0" src="vnos.gif" /></a>
...
while (iter.hasNext()) {
	...
	%>
	<tr><td class="opiscell" >
	<%=nul.jeNull(temc.getVrednost())%> (<%=nul.jeNull(temc.getTip())%>)</td><td>
	
	
	 <a href="Javascript:PGAPP()" onCLick="return godown('m_templateclass_class_vrednosti_hint.jsp?idp=<%=session.getAttribute("idp")%>&id=<%=temc.getId()%>&idtc=<%=sif.getId()%>&sifid=<%=sifid%>&opcija=spremeni&siframm=new&idback=<%=temc.getId()%>')"><img src="u.gif" border=0 title="Spremeni podatke"></a>
	 <a onClick="return vprasaj()" href="m_templateclass_class_hint.jsp?idp=<%=session.getAttribute("idp")%>&akcija=DELETE&idd=<%=temc.getId()%>&sifid=<%=sifid%>&kaj=vrednosti"><img src="x.gif" border=0 title="Brisi"></a>
...
<a onClick="return godown('m_templateclass_class_vrednosti_hint.jsp?idp=<%=session.getAttribute("idp")%>&opcija=new&siframm=new&sifrant=null&sifid=<%=sifid%>&idtc=<%=sif.getId()%>')" href="Javascript: PGAPP()"><img title="Dodaj novo vrednost" border="0" src="vnos.gif" /></a>
