...
if (request.getParameter("kaj")!=null && request.getParameter("kaj").equals("insert"))  {
	...
	sif.setBrisi(request.getParameter("brisi"));
	sif.setClass_name(request.getParameter("class"));
	sif.setTip_funkcije(request.getParameter("tip_funkcije"));
	
	if (!nul.jeNull(request.getParameter("idm")).equals("")) {
		sif.setId(new Long(request.getParameter("idm")));
	...
	dao.saveTemplateClassClass(sif);
...
document.location = "m_templatefunkcija_class2.jsp?kaj=insert&idm=<%=sif.getId()%>&sifid=<%=sifid%>&tip_funkcije="+document.getElementById("tip_funkcije").value+"&brisi="+document.getElementById("brisi").value+"&class="+document.getElementById("class_name").value;
	...
	<td width="30%" align="left">
  		 ...
  		 <select name="tip_funkcije" id="tip_funkcije" style="font-family: Verdana; font-size: 8pt" onChange="return shrani()" >
					...
					<option value="ER" <%=nul.jeNull(sif.getTip_funkcije()).equals("ER")?"selected":""%>>En rezultat</option>
						<option value="SU" <%=nul.jeNull(sif.getTip_funkcije()).equals("SU")?"selected":""%>>Sestej kolicino</option>
					<option value="CO" <%=nul.jeNull(sif.getTip_funkcije()).equals("CO")?"selected":""%>>Sestej zadetke</option>
					<option value="UU" <%=nul.jeNull(sif.getTip_funkcije()).equals("UU")?"selected":""%>>Update drugo tabelo</option>
					</select>
				...
				<%
					...
					if (nul.jeNull(sif.getTip_funkcije()).equals("LI") || nul.jeNull(sif.getTip_funkcije()).equals("ER") || nul.jeNull(sif.getTip_funkcije()).equals("IN")) {
						%>
						
						<br>
					Brisi iz tabele:<br>
  		 <select name="brisi" id="brisi" style="font-family: Verdana; font-size: 8pt" onChange="return shrani()" >
  		 <option value=""></option>
  		 
  		 	<%
					if (nul.jeNull(sif.getTip_funkcije()).equals("IN")) {
						%>
  		 <option value="IN" <%=nul.jeNull(sif.getBrisi()).equals("IN")?"selected":""%>>Kopiraj vse vrednosti iz najdenega objekta  </option>
  		 
  		 <%
					}
					else {
  		 %>
  		 <option value="BR" <%=nul.jeNull(sif.getBrisi()).equals("BR")?"selected":""%>>Brisanje (brisi zadetke pogojev)</option>
  		 		<%
					if (nul.jeNull(sif.getTip_funkcije()).equals("LI")) {
						%>
  		 <option value="UA" <%=nul.jeNull(sif.getBrisi()).equals("UA")?"selected":""%>>Update vse zadetke lista</option>
  		 <%
					}
					}
  		 %>
					...
					if (nul.jeNull(sif.getTip_funkcije()).equals("IN")) {
						%> (razen vez tabel in samo za isti objekt)
						...
						%>
						<input type="hidden" id="brisi" value="1">
						<%
					...
					<input type="button" value="Dodatni pogoji select"  style="font-family: Verdana; font-size: 8pt" onClick="godown('m_templateclassclass_pdf.jsp?id=<%=nul.jeNull(sif.getId()+"")%>')" >
...
while (iter.hasNext()) {
	...
	<%=nul.jeNull(temc.getIme_pogoja())%></td><td> = </td><td class="opiscell"><%=nul.jeNull(temc.getVrednost_pogoja())%> <b><%=nul.jeNull(temc.getIzvor_vrednosti())%></b></td>
	<td>
	 ...
	 <a onClick="return vprasaj()" href="m_templatefunkcija_class2.jsp?idp=<%=request.getParameter("idp")%>&akcija=DELETE&idd=<%=temc.getId()%>&sifid=<%=sifid%>&kaj=pogoji"><img src="x.gif" border=0 title="Brisi"></a>
...
while (iter.hasNext()) {
	...
	<%=nul.jeNull(temc.getVrednost())%> (<%=nul.jeNull(temc.getRabi_kot())%>)</td><td>
	 ...
	 <a href="Javascript:PGAPP()" onCLick="return godown('m_templatefunkcija_class_vrednosti2.jsp?idp=<%=request.getParameter("idp")%>&id=<%=temc.getId()%>&idtc=<%=sif.getId()%>&sifid=<%=sifid%>&opcija=spremeni&siframm=new&idback=<%=temc.getId()%>')"><img src="u.gif" border=0 title="Spremeni podatke"></a>
	 <a onClick="return vprasaj()" href="m_templatefunkcija_class2.jsp?idp=<%=request.getParameter("idp")%>&akcija=DELETE&idd=<%=temc.getId()%>&sifid=<%=sifid%>&kaj=vrednosti"><img src="x.gif" border=0 title="Brisi"></a>
...
<a onClick="return godown('m_templatefunkcija_class_vrednosti2.jsp?idp=<%=request.getParameter("idp")%>&opcija=new&siframm=new&sifrant=null&sifid=<%=sifid%>&idtc=<%=sif.getId()%>')" href="Javascript: PGAPP()"><img title="Dodaj novo vrednost" border="0" src="vnos.gif" /></a>
