...
while (iu.hasNext()) {
	...
	<td   width="70%" ><%=nul.jeNull(tcv.getVrednost())%>
		...
		%>
	<a onClick="return godown('m_templatefunkcija_pogoj_edit22.jsp?idv=<%=request.getParameter("idgumb")%>&opcija=new&siframm=new&sifrant=null&idt=<%=session.getAttribute("idt")%>&idg=<%=session.getAttribute("idg")%>&sifid=<%=tcv.getId()%>&vrednost=<%=tcv.getVrednost()%>')" href="Javascript: PGAPP()"><img title="Dodaj nov Pogoj" border="0" src="vnos.gif" /></a>	
