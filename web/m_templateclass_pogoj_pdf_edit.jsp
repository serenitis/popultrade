...
<form name=dod action="m_templateclass_pogoj_pdf_edit.jsp" method="post" onSubmit="return check()">
			...
			<option value="IF" <%=nul.jeNull(vseb.getPredpona_pogoja()).equals("IF")?"selected":""%>>IF</option>
				<option value="ELSE IF" <%=nul.jeNull(vseb.getPredpona_pogoja()).equals("ELSE IF")?"selected":""%>>ELSE IF</option>	
					...
					<option value="LIST" <%=nul.jeNull(vseb.getPredpona_pogoja()).equals("LIST")?"selected":""%>>LIST</option>
            		 ...
            		 <td class="opiscell" width="20%">Vrednost pogoja v primeru line break se vrednosti za vsako vrstico doda v tag &lt;bre&gt;&lt;/bre&gt;: </td><td class="valuecell" width="30%" colspan=3>
            		 
            			 <textarea name="vrednost_pogoja" style="font-family: Verdana; font-size: 8pt" rows="4" cols="100"><%=nul.jeNull(vseb.getVrednost_pogoja()+"")%></textarea>
            		 
            		 	  			 <input type="checkbox" name="uporabi_kot_neznanka" value="1" <%=nul.jeNull(vseb.getUporabi_kot_neznanka()+"").equals("1")?"checked":""%> style="font-family: Verdana; font-size: 8pt"> (uporabi isto kot v textboxu)
            		 <br> <select name="vrednost_pogojas" style="font-family: Verdana; font-size: 8pt" onChange="preseli()" >
				...
				//	thead = daoth.getTemplateHead(new Long(request.getParameter("sifid")));
            		...
            		<br><input type="text" name="funkcija_formata3" maxlength='550' size=110 value="<%=nul.jeNull(vseb.getFunkcija_formata3()+"")%>" style="font-family: Verdana; font-size: 8pt">vnese se dodatne parametre, ki potrebuje funkcija
